...
<div class="container">
    <h1> Gestione Clienti </h1>
        ...
        <div id="error_div" class="alert alert-danger">
            <p1> Errore nell'inserimento, assicurarsi di aver inserito tutti i dati correttamente</p1>
        </div>
        <div id="ins_ok" class="alert alert-success">
            <p1> Inserimento confermato </p1>
        </div>
        <div id="del_ok" class="alert alert-success">
            <p1> Eliminazione confermata </p1>
        </div>
        <!-- FINE ALERT -->
<!-- INSERIMENTO -->
    ...
    <table id="categories-table" class="table table-striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $c->surname_ref }}</td>
            <td>{{ $c->email_ref }}</td>
            <td><a href="#" class="btn btn-outline-secondary" id="edit-class-btn" data-name="{{ $c->name_ref }}">Modifica</a></td>
            <td><a href="#" class="btn btn-outline-danger" id="delete-btn" data-id="{{ $c->id }}" onclick="return confirm('Confermare eliminazione?');">Elimina Cliente</a></td>
          </tr>
          @endforeach
...
(function($) {
   $('document').ready(function(){
      ...
      $.ajax({
         ...
         success: function(data) {                        
               if (data.status === 'ok') {
                  ...
                  $('#categories-table').append(newRow);
      ...
      $('a#edit-class-btn').bind('click', function(e){
             e.preventDefault();
